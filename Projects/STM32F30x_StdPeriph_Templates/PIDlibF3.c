...
void setPIVelocity(void) {
	...
	setReferenceCurrent(velocityRegOut[0], velocityRegOut[1], velocityRegOut[2]);
}
...
void setPICurrent(void) {
	for (int i = 0; i < 3; i++) {
		currentRegError[i] = (referenceCurrent[i] - adcWartosc[i])
				* currentGainP;
	...
	setPWM(currentRegOut[0], currentRegOut[1], currentRegOut[2]);
}
